  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
        {guests.map((guest) => (
          <div key={guest.id} className="bg-white p-4 rounded-lg shadow-lg border border-gray-300">
            ...
            <p className="text-gray-600">Room Number: {guest.room_number || "N/A"}</p>
            <p className="text-gray-600">Room Type: {guest.room_type || "N/A"}</p>
            <p className="text-gray-600">Check-In: {formatDate(guest.check_in)}</p>
            <p className="text-gray-600">Check-Out: {formatDate(guest.check_out)}</p>
            <p className="text-gray-600">Amount: {guest.amount || "N/A"}</p>
            <p className="text-gray-600">Payment Date: {formatDate(guest.payment_date)}</p>
          </div>
        ))}
      </div>
    </div>
